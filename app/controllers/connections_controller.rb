class ConnectionsController < ApplicationController
  ...
  def fetch_connections
    ...
    @connection_list.each do |connection_item|
      ...
      @connection = Connection.new(customer_id: current_user.customer_id, connection_id: connection_item['id'].to_i,
                                   provider_code: connection_item['provider_code'], provider_name: connection_item['provider_name'])
      if @connection.save
    ...
    current_user.connections.each do |element|
      ...
      @accounts_list.each do |account_item|
        if Account.exists?(account_id: account_item['id'].to_i)
          ...
          @account = Account.new(account_id: account_item['id'].to_i,
                                 name: account_item['name'],
                                 nature: account_item['nature'],
                                 ballance: account_item['balance'].to_f,
                                 curency_code: account_item['currency_code'],
  ...
  def new
    begin
      response = RestClient::Request.execute(method: :post,
                                             ...
                                             payload: { 'data' => { 'customer_id' => current_user.customer_id.to_s,
                                                                    'return_connection_id' => false,
                                                                    'consent' => { 'scopes' => %w[account_details
